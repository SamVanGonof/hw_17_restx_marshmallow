...
# Инициализируем приложение, загружаем конфиг, подключаемся к бд.
app = Flask(__name__)
app.config.from_pyfile('config.py')
db.init_app(app)


# определяем "неймспэйсы"
api = Api(app)
...
# Определяем пути и представления основанные на классах для Movie
@movies_ns.route('/')
    ...
    def get(self):
        ...
        if director_id and genre_id:
            movies_found = db.session\
                ...
                .all()
            if not movies_found:
                return f"Нет найденных фильмов с director_id: {director_id}" \
                       f" и с genre_id: {genre_id}", 204
            else:
        ...
        if director_id:
            movies_found = db.session\
                ...
                .all()
            if not movies_found:
                return f"Нет найденных фильмов с director_id: {director_id}", 204
            else:
                return movies_schema.dump(movies_found), 200
        ...
        if genre_id:
            movies_found = db.session\
                ...
                .all()
            if not movies_found:
                return f"Нет найденных фильмов с genre_id: {genre_id}", 204
            else:
                return movies_schema.dump(movies_found), 200
...
# определяем пути и представления для Directors
@directors_ns.route('/')
...
class DirectorView(Resource):
    ...
    def put(self, uid):
        try:
            ...
            db.session.commit()
            db.session.close()
            return f"Режиссер с id: {uid} успешно обновлен", 201
    ...
    def delete(self, uid):
        ...
        if not director:
            ...
            db.session.commit()
            db.session.close()
            return f"Режиссер с id: {uid} успешно удален", 201


# Определяем пути и представления основанные на классах для Genres
@genres_ns.route('/')
...
class GenreView(Resource):
    ...
    def put(self, uid):
        try:
            ...
            db.session.commit()
            db.session.close()
            return f"Жанр с id: {uid} успешно обновлен", 201
    ...
    def delete(self, uid):
        ...
        if not genre:
            ...
            db.session.commit()
            db.session.close()
            return f"Жанр с id: {uid} успешно удален", 201
